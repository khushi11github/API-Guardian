...
function createProvider() {
    ...
    switch (aiProvider.toLowerCase()) {
        case 'openai': {
            ...
            if (!key) {
                logger_js_1.logger.warn('OPENAI_API_KEY not set, falling back to mock provider');
        ...
        case 'anthropic': {
            ...
            if (!key) {
                logger_js_1.logger.warn('ANTHROPIC_API_KEY not set, falling back to mock provider');
                return { provider: new MockAiProvider(), name: 'mock' };
            }
...
class AIService {
    ...
    async analyze(input) {
        logger_js_1.logger.info(`Running AI analysis with provider: ${this.providerName}`);
