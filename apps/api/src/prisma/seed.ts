import { PrismaClient, Environment, HttpMethod, ScheduleInterval, AssertionType, AssertionOperator, TestRunStatus, TriggerSource, IncidentStatus, IncidentSeverity, SpecType, ContractChangeType, LogLevel } from '@prisma/client';
...
async function main() {
  ...
  const healthyEndpoint = await prisma.endpoint.create({
    data: {
      ...
      timeoutMs: 5000,
      schedule: ScheduleInterval.EVERY_1_MIN,
      isActive: true,
      assertions: {
        create: [
          {
  ...
  const slowEndpoint = await prisma.endpoint.create({
    data: {
      ...
      expectedStatusCode: 200,
      timeoutMs: 10000,
      schedule: ScheduleInterval.EVERY_5_MIN,
      isActive: true,
      assertions: {
        create: [
          {
  ...
  const authEndpoint = await prisma.endpoint.create({
    data: {
      ...
      expectedStatusCode: 200,
      timeoutMs: 5000,
      schedule: ScheduleInterval.EVERY_15_MIN,
      isActive: true,
      authType: 'BASIC',
  ...
  for (let i = 24; i >= 0; i--) {
    ...
    testRunsData.push({
      ...
      assertionResults: [
        { type: AssertionType.STATUS_CODE, operator: AssertionOperator.EQUALS, expected: '200', actual: '200', passed: true, message: 'Status code 200 equals 200' },
        { type: AssertionType.RESPONSE_TIME, operator: AssertionOperator.LESS_THAN, expected: '800', actual: `${responseTimeMs}`, passed: true, message: `Response time ${responseTimeMs}ms < 800ms` },
      ],
  ...
  for (let i = 24; i >= 0; i--) {
    ...
    testRunsData.push({
      ...
      assertionResults: [
        { type: AssertionType.STATUS_CODE, operator: AssertionOperator.EQUALS, expected: '200', actual: '200', passed: true, message: 'Status code 200 equals 200' },
        { type: AssertionType.RESPONSE_TIME, operator: AssertionOperator.LESS_THAN, expected: '1000', actual: `${responseTimeMs}`, passed: false, message: `Expected response time less_than 1000ms, got ${responseTimeMs}ms` },
      ],
      triggeredBy: TriggerSource.SCHEDULER,
      timestamp,
    });
  ...
  for (let i = 24; i >= 0; i--) {
    ...
    testRunsData.push({
      ...
      status: isRecentFail ? TestRunStatus.FAILED : TestRunStatus.PASSED,
      ...
      assertionResults: isRecentFail
        ? [{ type: AssertionType.STATUS_CODE, operator: AssertionOperator.EQUALS, expected: '200', actual: '500', passed: false, message: 'Expected status equals 200, got 500' }]
        : [{ type: AssertionType.STATUS_CODE, operator: AssertionOperator.EQUALS, expected: '200', actual: '200', passed: true, message: 'Status code 200 equals 200' }],
      triggeredBy: TriggerSource.SCHEDULER,
      timestamp,
    });
    ...
    data: [
      {
        ...
        field: 'id',
        changeType: ContractChangeType.TYPE_CHANGED,
      ...
      },
      {
        contractId: updatedContract.id,
        field: 'role',
        changeType: ContractChangeType.FIELD_REMOVED,
  ...
  const latestFailRun = await prisma.testRun.findFirst({
    where: { endpointId: failingEndpoint.id, status: TestRunStatus.FAILED },
  ...
  const logEntries = [
    { level: LogLevel.INFO, service: 'scheduler', message: 'Triggered scheduled check for Get User Profile (1m interval)', metadata: { endpointId: healthyEndpoint.id } },
    { level: LogLevel.WARN, service: 'testing-engine', message: 'High response latency observed on Search Product Catalog: 1940ms > 1000ms threshold', metadata: { endpointId: slowEndpoint.id, latency: 1940 } },
    { level: LogLevel.ERROR, service: 'monitoring-worker', message: 'HTTP 500 Internal Server Error returned by Process Order Payment', metadata: { endpointId: failingEndpoint.id, status: 500, error: 'DB_CONNECTION_POOL_TIMEOUT' } },
    { level: LogLevel.CRITICAL, service: 'incident-system', message: 'Incident #1024 escalated to CRITICAL after 8 consecutive failures', metadata: { incidentId: activeIncident.id, failures: 8 } },
    { level: LogLevel.INFO, service: 'contract-differ', message: 'Contract change detected: field `id` changed from number to string, field `role` removed', metadata: { endpointId: healthyEndpoint.id } },
  ];
